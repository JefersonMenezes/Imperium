...
public class MainFXController implements Initializable {
    ...
    private void pegaDataCorrente() {
        ...
        for (int i = 0; i < meses.length; i++) {
            item = meses[i];
            listMesesAno.add(item);
        }
        cbMesAno.getItems().addAll(listMesesAno);
        System.err.println("Setar o valor >>>>" + custom.getMesNumero());
        cbMesAno.getSelectionModel().select(custom.getMesNumero() - 1);
    }
    ...
    private void atualizaMes() {
        ...
        LocalDate ld = LocalDate.of(dataCorrente.getYear(), mes, 1);
        dataCorrente = ld;
        atualiza();
    }
